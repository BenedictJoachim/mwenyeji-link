...
const Header = () => {
  ...
  return (
    <header className="bg-transparent p-4 text-white fixed w-full z-10">
      <div className="bg-blue-600 py-2 px-1 h-16 w-2/3 container mx-auto flex justify-between items-center rounded-full shadow-lg shadow-blue-500/50">
      ...
      {isOpen && (
        <div className={`fixed top-0 left-0 w-full h-full bg-blue-100 flex flex-col items-center justify-center sm:hidden transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <Link to="/" className="text-2xl text-blue-600 font-bold">Mwenyeji</Link>
          <button
